...
- Java helper 示例：`examples/java/nacos-registration/`
...
Java MCP Server 可参考 `examples/java/nacos-registration/`，其中包含：

- `NacosMcpServerRegistrar.java`：注册、注销、心跳和鉴权 token 获取。
- `McpServerNacosLifecycle.java`：`AutoCloseable` 生命周期封装。
- `RegisterMcpServerExample.java`：启动注册、shutdown hook 和心跳示例。
